...
const hPoint = new THREE.Vector3(-0.0010240999876032283, 3.66151538083604, 0.383484349790762);
...
function initParticles() {
	...
	for (let i = 0; i < PARTICLE_COUNT; i++) {
		...
		const pos = new THREE.Vector3().copy(spaceshipPos).add(forward.clone().multiplyScalar(d)).add(right.clone().multiplyScalar(offsetX)).add(up.clone().multiplyScalar(offsetY));
...
function updateParticles(delta) {
	...
	for (let i = 0; i < PARTICLE_COUNT; i++) {
		const currentPos = new THREE.Vector3(particlePositions[i * 3], particlePositions[i * 3 + 1], particlePositions[i * 3 + 2]);

		const velocityVec = new THREE.Vector3(particleVelocities[i * 3], particleVelocities[i * 3 + 1], particleVelocities[i * 3 + 2]);
		...
		if (d < SPAWN_DISTANCE_MIN || predictedPos.distanceTo(hPoint) < H_REINIT_DISTANCE) {
			...
			const newPos = new THREE.Vector3().copy(spaceshipPos).add(forward.clone().multiplyScalar(newD)).add(right.clone().multiplyScalar(offsetX)).add(up.clone().multiplyScalar(offsetY));
