...
def generate_qr_code(request, username):
    try:
        ...
        logging.error("An error occurred in generate_qr_code: %s", str(e))
...
def register_user(request):
    try:
        ...
        if email_error_or_encrypted in (
...
def get_user_id(request, username):
    try:
        ...
        logging.error("An error occurred in get_user_id: %s", str(e))
...
def get_game_history(request):
    try:
        ...
        try:
            ManualUser.objects.get(id=user_id)
        except ManualUser.DoesNotExist:
            return JsonResponse({"error": "User not found"}, status=404)
...
def upload_profile_picture(request):
    try:
        ...
        if "profile_picture" not in request.FILES:
            return JsonResponse({"success": False, "error": "Unauthorized : No file uploaded"}, status=200)
        file = request.FILES["profile_picture"]
        if file.size > MAX_FILE_SIZE:
            return JsonResponse({"success": False, "error": "Unauthorized : File too large"}, status=200)
        ext = os.path.splitext(file.name)[1].lower().strip(".")
        if ext not in ALLOWED_EXTENSIONS:
            return JsonResponse({"success": False, "error": "Unauthorized : Invalid file format"}, status=200)
        try:
            image = Image.open(file)
            image.verify()
        except Exception:
            return JsonResponse({"success": False, "error": "Unauthorized : Uploaded file is not a valid image"}, status=200)
...
def search_pilots(request):
    ...
    query = str(query)[:20] if len(query) > 20 else str(query)
    try:
        user = request.user
        if not user:
            return JsonResponse({"success": False, "error": "Unauthorized"}, status=200)
...
def get_leaderboard(request):
    try:
        ...
        logging.error("An error occurred in get_leaderboard: %s", str(e))
