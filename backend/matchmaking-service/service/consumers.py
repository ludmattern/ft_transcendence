...
class MatchmakingConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        logger.info(
            f"🔗 Connecté au groupe 'matchmaking_service' (channel={self.channel_name})"
        )
        ...
        if room_code:
            if action == "join":
                ...
                if result:
                    ...
                    logger.info(
                        f"📡 Private match (room={room_code}) ! Notif envoyée à user_{p1}"
                    )
                    ...
                    logger.info(
                        f"📡 Private match (room={room_code}) ! Notif envoyée à user_{p2}"
                    )
                else:
                    logger.info(
                        f"🔎 user_id={user_id} attend dans la private room {room_code}"
                    )
